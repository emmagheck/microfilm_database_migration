...
for record in root.findall("Microfilm_x0020_List"):
    ...
    if fields.get("THS") == "Y":
        data["accession_provenance"] = append_value(
            data.get("accession_provenance"), "Acquired from THS."
        )
    ...
    if restricted_value == "N":
        ...
        data["accession_access_restrictions_note"] = (
            "Some materials in this accession are restricted."
        )
    ...
    if fields.get("COLLECTION"):
        collection_value = fields["COLLECTION"]
        data["accession_title"] = collection_value

        title_lower = collection_value.lower()
        ...
        date_matches = re.findall(r"\b(\d{4})\b", cleaned_title)

        # Only proceed if 1 or 2 date patterns found
        if len(date_matches) == 1:
            data["date_2_begin"] = date_matches[0]
            data["date_2_type"] = "single"
            data["date_2_label"] = "Created"
        elif len(date_matches) == 2:
            data["date_2_begin"] = date_matches[0]
            data["date_2_end"] = date_matches[1]
            data["date_2_type"] = "inclusive"
            data["date_2_label"] = "Created"
        # Skip if more than 2 dates (do nothing)
